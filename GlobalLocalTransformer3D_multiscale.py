...
class GlobalLocalBrainAge(nn.Module):
    def __init__(self,inplace,
       ...
       # self.gloout2 = nn.Linear(128, 1)
        ...
        self.voting = nn.Conv1d(4,1,1,stride=1,padding=0)
       # self.finalout = nn.Linear(6, 1)
       # self.locout2 = nn.Linear(128, 1)
        
    def forward(self,xinput):
        #@+print(xinput.size())
        B,C,H,W,D =xinput.shape
        outlist = []
       ...
       # glo = self.gloout2(glo)
        ...
        # Can play with the parameters HERE######################################################################
        ...
        outlist2 = []
        for tensor in outlist:
            outlist2.append(tensor.clone())
        
        output = torch.stack(outlist2, dim=1)
        #print(output.shape)
