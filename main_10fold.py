...
parser.add_argument('--input-Fast', default=False, type=str2bool,
                    help='Do we have FastSurfer as input? If yes and if we have T1 it''s the second input. If yes and we do not have T1 it''s the first input.')
...
def main():
    ...
    print('Are we using Fast as input? : ', args.input_Fast)
    ...
    if args.double_vgg and (not args.input_T1 or not args.input_Fast):
        print('Not having both T1 and Fast as input. Argument double_vgg is meaningless. Setting it to False.')
    ...
    # args.resume = '/media/sail/HDD10T/DeepC_SCZ-Score/Pytorch_Models_For_Paper/FINAL_2023/2 DSx2_Resolution_YeTian_Multiscale_T1_2022_WB_T1_MNI152affine_BrainGluSchi_COBRE_3T_AllScan_811_TVT_COBRE_BRainGlu_NMorph_SE_Nets /result/model/spienaar10242023/checkpoint_best.tar'
    if args.resume:
    ...
    # Train
    Train_MRIDataset = MRIDataset(DataDir = TrainDataDir, mode = 'train', input_T1 = args.input_T1, input_DeepC = args.input_Fast, double_vgg = args.double_vgg,
    ...
    # Validation
    Validation_MRIDataset = MRIDataset(DataDir=ValidationDataDir, mode = 'validation', input_T1 = args.input_T1, input_DeepC = args.input_Fast, double_vgg = args.double_vgg,
                                    DeepC_isotropic = args.DeepC_isotropic, DeepC_isotropic_crop = args.DeepC_isotropic_crop, transform=transforms.Compose([ToTensor()]),
    ...
    # Test
    Test_MRIDataset = MRIDataset(DataDir=TestDataDir, mode = 'test', input_T1 = args.input_T1, input_DeepC = args.input_Fast, double_vgg = args.double_vgg,
                                    DeepC_isotropic = args.DeepC_isotropic, DeepC_isotropic_crop = args.DeepC_isotropic_crop, transform=transforms.Compose([ToTensor()]),
...
def train(train_loader, model, criterion, optimizer, epoch, device):
    ...
    for i, (data) in enumerate(train_loader):
        ...
        if args.input_Fast:
            input_data_DeepC = data['DeepC']
            

        # -----------------------------------------------------------------------------
        # --------------------- data_augmentation during training ---------------------
        # input torch size: [BS, Dim1, Dim2, Dim3]
        data_augmentation = 1
        ...
        if data_augmentation:
            ...
            tio.RandomNoise(p=0.6),                   # Gaussian noise 60% of times
            tio.OneOf({                                # either
            tio.RandomAffine(): 1,               # random affine
        ...
        if args.input_T1:
            input_data_T1 = torch.unsqueeze(input_data_T1,1) #data['T1'].unsqueeze(1)
            #print('input_data_T1.shape', input_data_T1.shape)
            #input_data_T1.shape torch.Size([5, 1, 65, 85, 60])

        if args.input_Fast:
            input_data_DeepC = torch.unsqueeze(input_data_DeepC,1) #data['DeepC'].unsqueeze(1)
        ...
        if args.data_dropout:
            ...
            # CHECKKKKK THISSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
    ...
    with torch.no_grad():
        for i, (data) in enumerate(val_loader):
            ...
            with torch.no_grad():
                ...
                # compute output; Trans4DC model
                mri_images = data['T1']
        
                # define batch loss
                output = torch.ones([len(mri_images),2])
                output = output.to(device)
                loss = []
        
		# loop through images in batch. model does not work just passing in the batch_size x 192 x 192 x 192 input.
                for cur_mri in range(0,len(mri_images)):
                    mri_image = mri_images[cur_mri].cuda(args.cuda_idx, non_blocking=True)
                    mri_image = torch.unsqueeze(mri_image,0)
                    mri_image = torch.unsqueeze(mri_image,0)
		    #print('model input mri_images shape is', mri_image.shape)
		    
		    # input: 1x1x192x192x192. output: 1x1728x512
                    batch_output = model(mri_image)[0].squeeze(0)
                    output[cur_mri,:]= batch_output

		    # target 
                    batch_target = target[cur_mri]
		    #print('model batch_target is', batch_target)
                    #batch_target = torch.unsqueeze(batch_target,0)
		    #print('model batch_target shape is', batch_target.shape)
		...
		# calculate average back loss and perform backprop
                avg_batch_loss = sum(loss)/len(loss)
                #optimizer.zero_grad()
...
def test(test_loader, model, criterion, epoch, device, scheduler = None, save_prediction_nifti = True):
    ...
    with torch.no_grad():
        ...
        if AUC >0.94:
            print('SAVING Test CSV', AUC)
            output_document = []
            for i in range(len(AD_ground_truth_array)):
                single_list = []
                single_list.append(AD_prediction_array[i])
                single_list.append(AD_ground_truth_array[i])
                output_document.append(single_list)
            import csv
            fields = ['Prediction', 'Ground Truth']
            rows = output_document

            output_csv = f'/media/sail/HDD10T/DeepC_SCZ-Score/Pytorch_Models_For_Paper/FINAL/2 DSx2_Resolution_YeTian_Multiscale_T1_2022_WB_T1_MNI152affine_BrainGluSchi_COBRE_3T_AllScan_811_TVT_COBRE_BRainGlu_NMorph_SE_Nets /result/csv/test.csv'
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    if is_best:
        ...
        print('=============================================================')
        print('Using AUC as the criterion for selecting best model. A new best model appears here.')
        print('=============================================================')
    else:
